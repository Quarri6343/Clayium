...
public class ClayRecipeBuilder<R extends ClayRecipeBuilder<R>> {

    protected ClayRecipeMap<R> recipeMap;

    protected final List<GTRecipeInput> inputs;
    protected final List<ItemStack> outputs;
    protected final List<ClayRecipe.ChanceEntry> chancedOutputs;

    protected final List<GTRecipeInput> fluidInputs;
    protected final List<FluidStack> fluidOutputs;

    protected int duration, CEt;
    protected boolean hidden = false;
    protected boolean isCTRecipe = false;
    protected int parallel = 0;
    protected int tier;
    protected Consumer<ClayRecipeBuilder<?>> onBuildAction = null;
    ...
    public ClayRecipeBuilder(ClayRecipe recipe, ClayRecipeMap<R> recipeMap) {
    ...
    protected ClayRecipeBuilder(ClayRecipeBuilder<R> recipeBuilder) {
        ...
        this.CEt = recipeBuilder.CEt;
        this.hidden = recipeBuilder.hidden;
        this.tier = recipeBuilder.tier;
    ...
    public R chancedOutput(ItemStack stack, int chance, int tierChanceBoost) {
        ...
        if (0 >= chance || chance > ClayRecipe.getMaxChancedValue()) {
            GTLog.logger.error("Chance cannot be less or equal to 0 or more than {}. Actual: {}.", ClayRecipe.getMaxChancedValue(), chance);
            GTLog.logger.error("Stacktrace:", new IllegalArgumentException());
            recipeStatus = EnumValidationResult.INVALID;
            return (R) this;
        }
        this.chancedOutputs.add(new ClayRecipe.ChanceEntry(stack.copy(), chance, tierChanceBoost));
        return (R) this;
    }
    ...
    public R chancedOutputs(List<ClayRecipe.ChanceEntry> chancedOutputs) {
        chancedOutputs.stream().map(ClayRecipe.ChanceEntry::copy).forEach(this.chancedOutputs::add);
        return (R) this;
    }
    ...
    public void chancedOutputsMultiply(ClayRecipe chancedOutputsFrom, int numberOfOperations) {
        for (ClayRecipe.ChanceEntry entry : chancedOutputsFrom.getChancedOutputs()) {
    ...
    /**
     * Appends the passed {@link ClayRecipe} onto the inputs and outputs, multiplied by the amount specified by multiplier
     * The duration of the multiplied {@link ClayRecipe} is also added to the current duration
     *
    ...
    public R append(ClayRecipe recipe, int multiplier, boolean multiplyDuration) {
        ...
        this.CEt(multiplyDuration ? recipe.getEUt() : this.CEt + recipe.getEUt() * multiplier);
    ...
    public R tier(int tier){
        this.tier = tier;
        return (R) this;
    }

    public R setRecipeMap(ClayRecipeMap<R> recipeMap) {
        this.recipeMap = recipeMap;
        return (R) this;
    }

    public R copy() {
        return (R) new ClayRecipeBuilder<>(this);
    }
    ...
    public ValidationResult<ClayRecipe> build() {
        return ValidationResult.newResult(finalizeAndValidate(), new ClayRecipe(inputs, outputs, chancedOutputs,
                fluidInputs, fluidOutputs, duration, CEt, hidden, isCTRecipe,
    ...
    protected R onBuild(Consumer<ClayRecipeBuilder<?>> consumer) {
    ...
    public void buildAndRegister() {
        ...
        ValidationResult<ClayRecipe> validationResult = build();
    ...
    public List<ClayRecipe.ChanceEntry> getChancedOutputs() {
        return chancedOutputs;
    }

    /**
     * Similar to {@link ClayRecipe#getAllItemOutputs()}, returns the recipe outputs and all chanced outputs
     *
    ...
    public List<ItemStack> getAllItemOutputs() {
        ...
        stacks.addAll(getChancedOutputs().stream().map(ClayRecipe.ChanceEntry::getItemStack).collect(Collectors.toList()));
