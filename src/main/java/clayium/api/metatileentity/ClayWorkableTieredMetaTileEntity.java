...
public abstract class ClayWorkableTieredMetaTileEntity extends ClayTieredMetaTileEntity implements IDataInfoProvider {

    protected static Set<FluidKey> fluidKeyCache;

    protected final ClayRecipeLogicEnergy workable;
    protected final ClayRecipeMap<?> recipeMap;
    ...
    public ClayWorkableTieredMetaTileEntity(ResourceLocation metaTileEntityId, ClayRecipeMap<?> recipeMap, ICubeRenderer renderer, int tier,
    ...
    protected void reinitializeEnergyContainer() {
        this.energyContainer = new ClayEnergyContainerHandler(this, (long)Math.pow(10, getTier()) * 1000, getTier());
    }
    ...
    public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip, boolean advanced) {
        tooltip.add(I18n.format("gui.Common.tier", getTier()));
    ...
    public List<ITextComponent> getDataInfo() {
        ...
        if (workable != null) {
            ...
            if (workable.getRecipeCEt() > 0) {
                list.add(new TextComponentTranslation("behavior.tricorder.workable_consumption",
                        new TextComponentTranslation(GTUtility.formatNumbers(workable.getRecipeCEt())).setStyle(new Style().setColor(TextFormatting.RED)),
                        new TextComponentTranslation(GTUtility.formatNumbers(workable.getRecipeCEt() == 0 ? 0 : 1)).setStyle(new Style().setColor(TextFormatting.RED))
                ));
            } else {
                list.add(new TextComponentTranslation("behavior.tricorder.workable_production",
                        new TextComponentTranslation(GTUtility.formatNumbers(workable.getRecipeCEt() * -1)).setStyle(new Style().setColor(TextFormatting.RED)),
                        new TextComponentTranslation(GTUtility.formatNumbers(workable.getRecipeCEt() == 0 ? 0 : 1)).setStyle(new Style().setColor(TextFormatting.RED))
                ));
            }
        }
