...
public class ClayRecipeMapCategory implements IRecipeCategory<ClayRecipeWrapper> {
    ...
    private static final HashMap<ClayRecipeMap<?>, ClayRecipeMapCategory> categoryMap = new HashMap<>();

    public ClayRecipeMapCategory(ClayRecipeMap<?> recipeMap, IGuiHelper guiHelper) {
    ...
    @Override
    public void setRecipe(IRecipeLayout recipeLayout, @Nonnull ClayRecipeWrapper recipeWrapper, @Nonnull IIngredients ingredients) {
    ...
    public static HashMap<ClayRecipeMap<?>, ClayRecipeMapCategory> getCategoryMap() {
        return categoryMap;
    }

    private static boolean shouldShiftWidgets(@Nonnull ClayRecipeMap<?> recipeMap) {
        return recipeMap.getMaxOutputs() >= 6 || recipeMap.getMaxInputs() >= 6 ||
                recipeMap.getMaxFluidOutputs() >= 6 || recipeMap.getMaxFluidInputs() >= 6;
    }

    private static int getPropertyShiftAmount(@Nonnull ClayRecipeMap<?> recipeMap) {
        int maxPropertyCount = 0;
        if (shouldShiftWidgets(recipeMap)) {
            for (ClayRecipe recipe : recipeMap.getRecipeList()) {
